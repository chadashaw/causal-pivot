...
create.power.df <- function(lrt.runs, params.ranges) {
  ...
  ranged.params <- paste0('param.', names(params.ranges))
...
logitG.lrt.runs <- run.sim.ranges(logitG.lrt, logitG.params, logitG.param.ranges, n.sims = 4)
logitG.power.df <- create.power.df(logitG.lrt.runs, logitG.param.ranges)
...
lrt.e.result <- run.lrt.sims(f.lrt.enriched.omega, params = params.e, n.sims = 1024) %>%
  as_tibble()
...
o.lrt.runs <- run.lrt.sims(o.f.lrt, o.params, n.sims = 32) %>%
  as_tibble()

o.power.df <- create.power.df(o.lrt.runs, o.param.ranges)
# saveRDS(o.power.df, file.path(out.dir, 'o.power.df.rds'))
...
lrt.result.beta <- mapply(function(gamma, beta) {
  params.beta$gamma <- gamma
  params.beta$beta <- beta
  
  r <- run.lrt.sims(f.lrt.beta, params.beta, n.sims = 4)
...
# lrt.result.beta <- readRDS(file.path('./.data/power_analysis/', 'beta.lrt.result.rds'))
...
power.plt.beta <- (
  ...
  scale_x_continuous(
    limits = c(0, max(gamma.range)),
    breaks = seq(0, max(gamma.range), length = 5),
    labels = f.scale.x
  ) +
  labs(
    x = expression(gamma),
...
logitG_A.param.ranges

logitG_A.lrt.runs <- run.sim.ranges(logitG_A.f.lrt, logitG_A.params, logitG_A.param.ranges, n.sims = 2^8)
logitG_A.power.df <- create.power.df(logitG_A.lrt.runs, logitG_A.param.ranges)
...
compare.df <- compare.df %>% filter(test == 'co.lrt')
...
(
  ...
  save.plot('compare.lrt.power', root.dir = out.dir, w = 4.5, h = 4.5)
```
