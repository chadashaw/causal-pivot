...
```{r excute_beta_lrt_sims, cache=T}
source('../lrt_equations/equations.logitG.R')
list2env(lrt.logitG.equations, envir = environment())
...
lrt.result.beta <- mapply(function(beta, eta) {
  ...
  r <- run.lrt(f.lrt.beta, params.beta, n.sims = n.sims)
...
gamma.range <- seq(0.0, 2.6, length.out = 8)
beta.range <- c(0.1, 0.3, 0.6)
...
power.plt.beta <- (
  ...
  scale_linetype_manual(values = c('0.6' = 'solid', '0.3' = 'dashed', '0.1' = 'dotted')) +
