...
```{r lrt_logitg_a_3_cohorts, cache=t}
# source mle functions and load into local environment ####
...
logitg.age.low.lrt.result <- postprocess.lrt.run(logitG.age.low.lrt.run)

logitg.age.high.lrt.run <- purrr::imap(cohort.dfs, function(cohort.df, cohort.name) {
  print(paste0('logitg.age.high: ', cohort.name))
  purrr::map(cohort.lkps$gene.grp, function(gene.grp) {
    print(paste0('gene.grp: ', gene.grp))
    lrt.data <- make.lrt.data(rv.prs.df, cohort.df, gene.grp) %>%
      mutate(age.half = ntile(a, 2)) %>%
...
logitg.age.high.lrt.result <- postprocess.lrt.run(logitg.age.high.lrt.run)

# save result

saverds(logitg.age.low.lrt.result, file.path(out_dir, 'logitg.age.low.mle.result.rds'))
saverds(logitg.age.high.lrt.result, file.path(out_dir, 'logitg.age.high.mle.result.rds'))
```
