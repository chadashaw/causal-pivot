...
# saveRDS(knn.results, file.path(OUT_DIR, 'knn.results.rds'))
...
a.cohort.dfs %>%
  purrr::map(~{
    .x <- filter(.x, case)
    .x$age <- scale(.x$age)
    .x$prs.res <- lm(prs ~ age, data = .x)$residuals
    
    .x %>%
      ggplot(aes(x = prs, y = prs.res)) +
      geom_point() +
...

{
  ...
  save.plot('fig4', root.dir = PLOTS_DIR, sub.dir = 'composed', w = 6.5, h = 5)

{
  ...
  save.plot('fig4.w.age', root.dir = PLOTS_DIR, sub.dir = 'composed', w = 6.5, h = 5)
```
...
```{r inverse_probability_weighting, cache=T}
# set parameters ####
n.perms <- 1000 # number of permutations for permutation testing
...
a.ipw.plts <- lapply(names(cohorts), function(cohort.name) {
  hist.plt <- a.ipw.results[[cohort.name]]$hist.plt
  imp.acc.plt <- a.propensity[[cohort.name]]$imp.acc.plt
  imp.gini.plt <- a.propensity[[cohort.name]]$imp.gini.plt
  list(
...
save.plot(a.ipw.composed.plt, paste('ancestry.ipw', sep='.'), w = 6 * 2, h = 4.5 * 2, root.dir = PLOTS_DIR, sub.dir = 'ancestry.balancing/cv.rfm.propensity')

purrr::map(a.ipw.results, ~.$p.val)
saveRDS(a.ipw.results, file = file.path(OUT_DIR, 'a.ipw.cv.results.rds'))
```
...
# 
# r = 0.637 # the "decay" factor to simulate decreasing eigenvalues of PCs
# pc.decay <- exp(-r * 0:(n.pcs - 1))
# pc.weights <- pc.decay / sum(pc.decay)

cv.glm.prop <- purrr::imap(a.cohort.dfs, function(cohort.df, cohort.name) {
  x <- inner_join(
    cohort.df,
    get.pcs.df(n.pcs = n.pcs),
    by = 'sample.id'
  ...
  model <- glm(rv ~ ., family = binomial(link = 'logit'), data = select(x, rv, starts_with('pc.')))
  
  x$p <- predict(model, x, type = "response")
  x$p <- reg.p(x$p, t = 4, epsilon = 0.001)
  x
})

purrr::map(cv.glm.prop, function(prop) {
  prop %>%
    summarize(min = min(p), max = max(p))
}) %>%
  concat.df.list(key.cols = 'cohort')

purrr::imap(cv.glm.prop, function(prop, cohort.name) {
  prop %>%
    # mutate(p = reg.p(p, t = 4)) %>%
    # mutate(p.bin = ntile(p, 5)) %>%
    overlap.plt() +
