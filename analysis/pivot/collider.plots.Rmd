...
tertiles.p.vals <- pmap(age.tertiles.grid, function(my.cohort.name, gene.grp.name, age.n.tile) {
  ...
  as_tibble() %>%
  rowwise() %>%
  mutate(stars = p.val.stars(p.val)) %>%
  ungroup() %>%
  left_join(
    n.events.df %>%
...
for (age.n.tile in seq(n.tiles)) {
  ...
  age.tertiles.3x3[[age.n.tile]] <- (
  age.n.tile.events.df %>%
    ggplot(aes(x = factor(prs.qnt), y = n)) +
    geom_bar(
      stat = "identity",
      color = 'black',
    ...
    facet_grid(rows = vars(cohort), cols = vars(gene.grp), scales = "free_y") +
    scale_y_continuous(expand = expansion(mult = c(0, 0.5))) +
    geom_text(data = age.n.tile.p.vals, aes(label = stars, y = y), x = 3, vjust = -1/3, size = 6, show.legend=F) +
    theme_bw() +
    theme(
      panel.grid = element_blank(),
      axis.text = element_text(size = 12, face = "bold"),
    ...
    labs(
     ...
     save.plot(paste0('age.3x3.tertiles.', age.n.tile), w = 4.5 * 2, h = 4.5 * 2, root.dir = PLOTS_DIR, sub.dir = 'collider')
}
