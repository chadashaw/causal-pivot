...
mt.dist <- (
  ...
  geom_text(data = max.counts, aes(label = label, y = max.n), x = 4)
) %>%
...
n.mle.perms <- 1024 # number of permutations

logitG.lrt.run <- purrr::map(cohort.dfs, function(cohort.df) {
  purrr::map(cohort.gene.grps, function(gene.grp) {
    # massage data
    ...
    lrt.target.model <- broom::tidy(glm(Y ~ G * X, family = binomial(link = 'logit'), data = lrt.df))
    ...
    params <- list(
      ...
      gamma = 0,
      eta = 0,
      omega = mean(lrt.df$G) # rare variant frequency in disease
    )

    # calculate MLE w/ some jitter (internal to run.lrt) to avoid extreme results
    a <- lapply(seq(16), function(i) {
    ...
    perm.result <- lapply(seq(n.mle.perms), function(i) {
      run.lrt(as.list(mutate(lrt.df, G = sample(G))), params, cases.only=T)
    }) %>%
      do.call(bind_rows, .) %>%
      as.data.frame()
...
logitG.lrt.run <- logitG.lrt.run.bak
for (cohort.name in names(logitG.lrt.run)) {
  for (gene.grp in cohort.gene.grps) {
    logitG.lrt.run[[cohort.name]][[gene.grp]]$mle.run <- logitG.lrt.run[[cohort.name]][[gene.grp]]$mle.result
    logitG.lrt.run[[cohort.name]][[gene.grp]]$mle.result <- NULL
  }
}

# c('params', 'mle.target', 'mle.run', 'perm.result') %>%
c('params', 'mle.run', 'perm.result') %>%
# c('mle.target') %>%
  purrr::set_names(paste('logitG', ., sep = '.')) %>%
  purrr::map(function(leaf.key) {
    concat.df.list(extract.leaf(logitG.lrt.run, leaf.key), key.cols = c('cohort.name', 'gene.grp')) %>%
      mutate(
        cohort.name = factor(cohort.name, levels = names(COHORTS)),
        gene.grp = factor(gene.grp, levels = cohort.gene.grps)
      )
  }) %>%
  list2env(mle.objs, envir = .GlobalEnv)

logitG.fisher.information <- extract.leaf(logitG.lrt.run, 'fisher.information')  

# sometimes the optimization produces unrealistic gamma/eta estimates
logitG.perm.result <- logitG.perm.result %>%
...
logitG.conf.int <- logitG.fisher.information %>%
  purrr::imap(function(a, cohort.name) {
    a %>%
      purrr::imap(function(b, gene.grp) {
        x <- solve(b$I1 * b$n1 + b$I2 * b$n2)
        y <- logitG.mle.run[
          logitG.mle.run$cohort.name == cohort.name &
            logitG.mle.run$gene.grp == gene.grp,]
        data.frame(
...
logitG.perm.q95 <- logitG.perm.result %>%
  group_by(cohort.name, gene.grp) %>%
  summarize(ll = q.95 <- quantile(ll, 0.95))

# calculate p.values for the cohorts based on 95th quantile of LL permutations
logitG.mle.p.vals <- inner_join(logitG.mle.run, logitG.perm.result, by = c('cohort.name', 'gene.grp'), suffix = c('.mle', '.perm')) %>%
  group_by(cohort.name, gene.grp) %>%
...
(
  ...
  geom_vline(aes(xintercept = ll), linewidth = 1, linetype = 'dashed', color = '#f42e3d', data = logitG.mle.run) +
  geom_vline(aes(xintercept = ll), linewidth = 1, linetype = 'dashed', data = logitG.perm.q95) +
  labs(
    y = 'Count',
    x = 'Log Likelihood',
  )
...
(
  ...
  c(
    lapply(gene.grps, function(my.gene.grp) {
      mle.perm <- logitG.perm.result %>%
        filter(gene.grp == my.gene.grp & cohort.name == my.cohort.name)
      mle.observed <- logitG.mle.run %>%
        filter(gene.grp == my.gene.grp & cohort.name == my.cohort.name)
      mle.target <- logitG.mle.target %>%
        filter(gene.grp == my.gene.grp & cohort.name == my.cohort.name) %>%
      ...
      mle.perm %>%
        ...
        geom_density_2d(color = "black", alpha = 0.7, bins = 7) +
        ...
        geom_point(x = mle.target$gamma, y = mle.target$eta,
          color = "black", size = 4, shape = 4, stroke = 1) +
  ...
  {
    c(map(gene.grps, ~grid::textGrob(.x)),
      list(plot_spacer()),
      .
    )
  } %>%
  ...
  save.plot('logitG.MLE.permutation.contour', root.dir = PLOTS_DIR, sub.dir = 'maximum.likelihood.estimation', w = 4.5 * 2, h = 4.5 * 2)

# save results ####
logitG.lrt.result <- list(
  params = logitG.params,
  mle.result = logitG.mle.run,
  mle.p.vals = logitG.mle.p.vals,
  perm.result = logitG.perm.result,
  fisher.information = logitG.fisher.information,
  logitG.conf.int = logitG.conf.int
)
...
```{r lrt_liabilityG_HC190, cache=T}
# this can be computationally intensive, so for the sake of reasonable knitting
# times, we cache the result
...
liabilityG.lrt.run <- purrr::map(unname(cohort.gene.grps), function(gene.grp) {
  # massage data
  G.df <- rv.prs.df[c('sample.id', gene.grp)]
  ...
  mle.run <- run.lrt(as.list(lrt.df), params, cases.only=T)
...
for (gene.grp in cohort.gene.grps) {
  liabilityG.lrt.run[[gene.grp]]$mle.run <- liabilityG.lrt.run[[gene.grp]]$mle.result
  liabilityG.lrt.run[[gene.grp]]$mle.result <- NULL
}
...
# c('mle.target') %>%
  ...
  purrr::map(function(leaf.key) {
    concat.df.list(extract.leaf(liabilityG.lrt.run, leaf.key), key.cols = c('gene.grp')) %>%
      mutate(gene.grp = factor(gene.grp, levels = cohort.gene.grps))
  }) %>%
  list2env(mle.objs, envir = .GlobalEnv)

# liabilityG.fisher.information <- extract.leaf(liabilityG.lrt.run, 'fisher.information')  
...
(
  ...
  geom_histogram(bins = 14) +
  geom_vline(aes(xintercept = ll), linewidth = 1, linetype = 'dashed', color = '#f42e3d', data = liabilityG.mle.run) +
  geom_vline(aes(xintercept = ll), linewidth = 1, linetype = 'dashed', data = liabilityG.perm.q95) +
  labs(
    y = 'Count',
    x = 'Log Likelihood',
  )
...
n.events.df$cohort <- factor(n.events.df$cohort, levels=names(COHORTS))
...
panel.b <- tertiles.3x3 + composed.theme + scale_y_continuous(labels = function(x) x)
panel.c <- mt.dist + composed.theme +
  theme(
...
(
  (((panel.a / panel.b) + plot_layout(heights = c(2, 8))) | panel.c) /
    guide_area() +
    plot_layout(heights = c(6, 1)) +
    plot_annotation(tag_levels = 'a', tag_suffix = ')') +
    plot_layout(guides = 'collect')
) %>%
  save.plot('fig3', root.dir = PLOTS_DIR, sub.dir = 'composed')
  
list(
  case.control.GvX = cc.prs.stats.df,
  tertile.pos.samples = n.events.df,
  rv.mt.alt.counts = alt.counts
) %>%
  saveRDS(file.path(OUT_DIR, 'fig3.RDS'))
```
...
saveRDS(modifyList(knn.results, knn.plots), file = file.path(OUT_DIR, 'fig4.rds'))
...
for (cohort.name in names(COHORTS)) {
  ...
  hist.plt <- data.frame(x = p.dist.std) %>%
    ...
    geom_vline(xintercept = z, linetype = "dashed", color = "red", size = 1.5) +
    geom_vline(xintercept = quantile(p.dist.std, 0.05), linetype = "dashed", color = "black", size = 1.5) +
    scale_x_continuous(expand = c(0,0)) +
    scale_y_continuous(expand = c(0,0)) +
    theme_classic() +
    theme(
...
(
  ...
  save.plot('fig5', root.dir = PLOTS_DIR, sub.dir = 'composed')
