...
sim.run.dir <- '.data/sim_runs/fig2A/'

logitG.sim.runs <- dir(sim.run.dir) %>%
  .[grepl('64.rds$', .)] %>%
  purrr::map(~ readRDS(file.path(sim.run.dir, .x)))

params.logitG <- logitG.sim.runs[[1]]$params

logitG.lrt.runs <- logitG.sim.runs %>%
...
```{r save_params}
list(
  logitG.params = unlist(params.logitG),
