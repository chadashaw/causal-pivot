...
colnames(lsd.dbl) <- gsub('GBA', 'GBA1', colnames(lsd.dbl))
...
```
```{r lsd_burden_plots, warning=F, message=F}
...
lsd.dbl.heatmap <- lsd.pairs.long %>%
  ...
  theme(
    aspect.ratio = 1,
    panel.grid = element_blank(),
    axis.text.x = element_text(size = 7, face = "bold.italic", angle = 45, hjust = 1),
...
rm(list=c('annotations', 'geno.mtx'))
gc()
```