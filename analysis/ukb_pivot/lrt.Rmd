---
title: "LogitG Likelihood Ratio Test"
...
library(paletteer, quietly = F, warn.conflicts = F)
library(waffle, quietly = F, warn.conflicts = F)
...
Apply the Causal Pivot maximum likelihood estimation (MLE) procedure
...
are used to derive estimates for model parameters.

MLE is performed via `optim()` using the built-in `SANN` simulated
annealing algorithm. To correct for variability found in some areas of
the parameter space, MLE is performed multiple times and the
median result is used.

Permutation analysis of the MLE statistic under G (the
...
- Lower permutation count reduces computation time but increases variability
    ...
    c(
      lapply(gene.lkps$grp.to.genes, function(my.gene.grp) {
        mle.perm <- lrt.result$mle.perm %>%
          ...
          filter(abs(gamma) <= 2.5 & abs(eta) <= 1.0)
...
The equations for the LogitG model are given in
[equations.logitG.R](../equations/equations.logitG.R).
...
```{r logitG_plots, warning=F, message=F}
logitG.lrt.result <- readRDS(file.path(OUT_DIR, 'logitG.mle.result.rds'))

plot.mle.perm.contour(logitG.lrt.result) %>%
  save.plot('supp.fig6', root.dir = PLOTS_DIR, sub.dir = 'supplemental', w = 6.5, h = 6.5)
...
The equations for the LogitG model with A (age) are given in
[equations.logitG_A.R](../equations/equations.logitG_A.R).

### Perform LogitG_A Permutations

```{r lrt_logitG_A_3_cohorts}
# source mle functions and load into local environment ####
source('../equations/equations.logitG_A.R')
...
```{r lrt_logitG_A_plots, warning=F, message=F}
...
## Age-Partitioned LRT Permutations

Perform age-stratified analysis by partitioning cohorts into
age tertiles and running separate LRT analyses.

```{r lrt_logitG_age_partition_cohorts, warning=F}
# source mle functions and load into local environment ####
source('../equations/equations.logitG.R')
logitG.lrt.equations <- define.logitG.equations(cases.only=T)
...
```{r logitG_age_split_plots, warning=F, message=F}
